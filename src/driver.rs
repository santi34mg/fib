...
///
/// Tries each `search_roots` entry in order. For each root it first tries the
/// full path (`["std","io"]` -> `<root>/std/io.fib`) and then — for roots
/// that *are* the top namespace (e.g. `import std::io` with `-I std`) — the
/// path with the first segment dropped (`<root>/io.fib`).
pub fn load_module_source(
    ...
    for root in search_roots {
        ...
        match fs::read_to_string(&full) {
            Ok(src) => return Ok((full, src)),
            Err(e) => {
                attempted_file.get_or_insert((full, e));
            }
        }
        ...
        if import_path.len() > 1 {
            ...
            match fs::read_to_string(&short) {
                Ok(src) => return Ok((short, src)),
                Err(e) => {
                    if attempted_file.is_none() {
                        attempted_file = Some((short, e));
                    }
                }
            }
        }
...
fn resolve_module_recursive(
    ...
    let result = (|| {
        let (file_path, source) = load_module_source(path, search_roots)?;
...
pub fn run_frontend(opts: &CompilationOptions) -> Result<FrontendResponse, DriverError> {
    ...
    let source = read_source(file, opts.source_override.as_deref())?;
    ...
    let ast = parse_source(tokens.clone(), file, &source)?;
...
/// -> link. Frontend-only emits (`lex|parse|typed`, `--check`) never touch
/// LLVM or clang, so they work without the `llvm` feature.
pub fn compile(opts: &CompilationOptions) -> Result<CompileOutput, DriverError> {
