...
use crate::frontend::ast::{Ast, declaration::DeclarationNode, expression::Expression, type_expression::TypeExpression};
    ...
    fn module_statements(ast: &Ast) -> Vec<&StatementKind> {
    ...
    fn test_multi_var_declaration_inferred_type() {
        ...
        let stmts = module_statements(&ast);
        assert_eq!(stmts.len(), 1);
        if let StatementKind::MultiVariableDeclaration {
            identifiers,
            values,
        } = stmts[0]
        {
            assert_eq!(identifiers.len(), 2);
            assert_eq!(identifiers[0].value, "q");
            assert_eq!(identifiers[1].value, "r");
    ...
    fn test_if_else_statement() {
        ...
        let stmts = module_statements(&ast);
        assert_eq!(stmts.len(), 1);
        if let StatementKind::If { else_branch, .. } = stmts[0] {
    ...
    fn test_function_declaration_no_params() {
        ...
        assert_eq!(func.signature.name.value, "greet");
    ...
    fn test_function_declaration_with_params() {
        ...
        let func = func.expect("expected function");
        assert_eq!(func.signature.name.value, "add");
        assert_eq!(func.signature.parameters.len(), 2);
        assert_eq!(func.signature.parameters[0].parameter_name.value, "a");
        assert_eq!(func.signature.parameters[1].parameter_name.value, "b");
